...
impl RuleParser {
    ...
    fn build_ast(pair: pest::iterators::Pair<Rule>) -> RuleResult<AstNode> {
        match pair.as_rule() {
            ...
            Rule::or_expr => {
                Self::build_binary_expr_generic(pair)
            }
            ...
            Rule::request_context => {
                Ok(AstNode::Identifier("@request".to_string()))
            }
            ...
            Rule::identifier => {
                Ok(AstNode::Identifier(pair.as_str().to_string()))
            }
            
            // Operator rules - these should be handled by their parent expressions
            Rule::or_op | Rule::and_op | Rule::in_op | Rule::eq_op | 
            Rule::rel_op | Rule::add_op | Rule::mul_op | Rule::unary_op => {
                Err(RuleError::Parse(format!("Operator rule should not be parsed directly: {:?}", pair.as_rule())))
            }
            
            _ => Err(RuleError::Parse(format!("Unexpected rule: {:?}", pair.as_rule()))),
        }
    }
    ...
    fn parse_binary_operator(pair: &pest::iterators::Pair<Rule>) -> RuleResult<BinaryOperator> {
        match pair.as_str() {
            ...
            _ => Err(RuleError::Parse(format!("Unknown operator: {}", pair.as_str()))),
        }
    }
}
...
mod tests {
    ...
    fn test_parse_member_access() {
        ...
        assert_eq!(ast, AstNode::MemberAccess {
    ...
    fn test_parse_array_literal() {
        ...
        assert_eq!(ast, AstNode::Array(vec![
    ...
    fn test_parse_complex_expression() {
        let rule = "record.published == true || (user.role == \"admin\" && record.owner == user.id)";
