...
impl ExpressionEvaluator {
    ...
    pub fn evaluate(ast: &AstNode, context: &EvaluationContext) -> RuleResult<Value> {
        match ast {
            ...
            AstNode::Identifier(name) => Self::resolve_identifier(name, context),
        ...
        match name {
            ...
            "user" => {
                if let Some(user) = &context.user {
                    Ok(serde_json::to_value(user).map_err(|e| {
                        RuleError::Evaluation(format!("Failed to serialize user: {}", e))
                    })?)
                } else {
                    Err(RuleError::Evaluation("No user in context".to_string()))
                }
            }
            "@request" => Ok(serde_json::to_value(&context.request).map_err(|e| {
                RuleError::Evaluation(format!("Failed to serialize request: {}", e))
            })?),
    ...
    fn evaluate_unary_op(
        ...
        match operator {
            UnaryOperator::Not => Ok(Value::Bool(!Self::is_truthy(&operand_val))),
        ...
        match name {
            "size" | "length" => {
                ...
                match arg_val {
                    ...
                    _ => Err(RuleError::Evaluation(
                        "Cannot get size of non-collection value".to_string(),
                    )),
                }
            }
            ...
            "contains" => {
                ...
                match container {
                    Value::Array(arr) => Ok(Value::Bool(
                        arr.iter().any(|v| Self::values_equal(v, &item)),
                    )),
                    Value::String(s) => {
                    ...
                    _ => Err(RuleError::Evaluation(
                        "Cannot check contains on non-collection value".to_string(),
                    )),
                }
            }
    ...
    fn add_values(left: &Value, right: &Value) -> RuleResult<Value> {
        match (left, right) {
            (Value::Number(a), Value::Number(b)) => {
                ...
                let b_f64 = b.as_f64().unwrap_or(0.0);
                Ok(Value::Number(
                    serde_json::Number::from_f64(a_f64 + b_f64).unwrap(),
                ))
            }
    ...
    fn subtract_values(left: &Value, right: &Value) -> RuleResult<Value> {
        match (left, right) {
            ...
            _ => Err(RuleError::Evaluation(
                "Cannot subtract non-number values".to_string(),
            )),
        }
    }
    ...
    fn multiply_values(left: &Value, right: &Value) -> RuleResult<Value> {
        match (left, right) {
            ...
            _ => Err(RuleError::Evaluation(
                "Cannot multiply non-number values".to_string(),
            )),
        }
    }
    ...
    fn divide_values(left: &Value, right: &Value) -> RuleResult<Value> {
        match (left, right) {
            ...
            _ => Err(RuleError::Evaluation(
                "Cannot divide non-number values".to_string(),
            )),
        }
    }
    ...
    fn modulo_values(left: &Value, right: &Value) -> RuleResult<Value> {
        match (left, right) {
            ...
            _ => Err(RuleError::Evaluation(
                "Cannot modulo non-number values".to_string(),
            )),
        }
    }
    ...
    fn in_operation(left: &Value, right: &Value) -> RuleResult<Value> {
        match right {
            Value::Array(arr) => Ok(Value::Bool(arr.iter().any(|v| Self::values_equal(left, v)))),
            Value::String(s) => {
            ...
            _ => Err(RuleError::Evaluation(
                "'in' operator requires array or string on right side".to_string(),
            )),
        }
    }
}
...
mod tests {
    ...
    fn test_evaluate_logical_operations() {
        ...
        let ast =
            RuleParser::parse_rule("user.role == \"user\" || record.published == true").unwrap();
        let result = ExpressionEvaluator::evaluate(&ast, &context).unwrap();
        assert_eq!(result, Value::Bool(true));
    }
