...
impl CsrfTokenStore {
    ...
    pub async fn validate_token(&self, token: &str, session_id: Option<&str>) -> bool {
        ...
        if let Some(csrf_token) = tokens.get(token) {
            ...
            if let (Some(stored_session), Some(provided_session)) = (&csrf_token.session_id, session_id) {
    ...
    async fn cleanup_expired_tokens(&self, tokens: &mut HashMap<String, CsrfToken>, now: u64) {
        tokens.retain(|_, csrf_token| {
            now - csrf_token.created_at <= self.config.token_lifetime
        });
    }
    ...
    pub async fn cleanup_task(&self) {
        ...
        loop {
            ...
            if initial_count != final_count {
                debug!("Cleaned up {} expired CSRF tokens", initial_count - final_count);
            }
        }
    }
}
...
pub async fn csrf_protection_middleware(
    ...
    if matches!(method, Method::GET | Method::HEAD | Method::OPTIONS) 
        || csrf_store.config.exempt_paths.iter().any(|exempt_path| path.starts_with(exempt_path)) {
    ...
    let csrf_token = headers
        ...
        .or_else(|| {
            ...
            headers
                ...
                .and_then(|cookies| extract_csrf_token_from_cookies(cookies, &csrf_store.config.cookie_name))
        });

    let csrf_token = match csrf_token {
        Some(token) => token,
        None => {
            warn!("CSRF protection: No CSRF token provided for {} {}", method, path);
    ...
    // Validate CSRF token
    if !csrf_store.validate_token(csrf_token, session_id.as_deref()).await {
        warn!("CSRF protection: Invalid CSRF token for {} {}", method, path);
        return Err(StatusCode::FORBIDDEN);
    }
...
mod tests {
    ...
    async fn test_csrf_token_generation_and_validation() {
        ...
        assert!(!store.validate_token("invalid_token", Some("session123")).await);
    }
    ...
    fn test_cookie_extraction() {
        ...
        assert_eq!(extract_csrf_token_from_cookies(cookies, "csrf_token"), Some("def456"));
        assert_eq!(extract_csrf_token_from_cookies(cookies, "session_id"), Some("abc123"));
        assert_eq!(extract_csrf_token_from_cookies(cookies, "nonexistent"), None);
    }
