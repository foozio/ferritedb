...
impl CollectionService {
    ...
    pub async fn create_collection(&self, request: CreateCollectionRequest) -> CoreResult<Collection> {
    ...
    pub async fn create_collection_metadata_only(&self, request: CreateCollectionRequest) -> CoreResult<Collection> {
        self.create_collection(request).await
    }

    /// Add a field to an existing collection
    pub async fn add_field(&self, collection_name: &str, field_request: CreateFieldRequest) -> CoreResult<Collection> {
        let mut collection = self.repository.find_by_name(collection_name).await?
        ...
        if collection.schema_json.get_field(&field_request.name).is_some() {
        ...
        let field = if field_request.required { field.required() } else { field };
        let field = if field_request.unique_constraint { field.unique() } else { field };
        ...
        collection.schema_json.json_schema = Some(self.generate_json_schema(&collection.schema_json)?);
    ...
    pub async fn remove_field(&self, collection_name: &str, field_name: &str) -> CoreResult<Collection> {
        let mut collection = self.repository.find_by_name(collection_name).await?
            .ok_or_else(|| CoreError::CollectionNotFound(collection_name.to_string()))?;
    ...
    pub async fn update_field(&self, collection_name: &str, field_name: &str, field_request: CreateFieldRequest) -> CoreResult<Collection> {
        let mut collection = self.repository.find_by_name(collection_name).await?
            .ok_or_else(|| CoreError::CollectionNotFound(collection_name.to_string()))?;
    ...
    pub async fn delete_collection(&self, name: &str) -> CoreResult<bool> {
        let collection = self.repository.find_by_name(name).await?
    ...
    pub async fn validate_record_data(&self, collection_name: &str, data: &Value) -> CoreResult<()> {
        let collection = self.repository.find_by_name(collection_name).await?
            .ok_or_else(|| CoreError::CollectionNotFound(collection_name.to_string()))?;
    ...
    fn validate_collection_name(&self, name: &str) -> CoreResult<()> {
        if name.is_empty() {
            return Err(CoreError::ValidationError("Collection name cannot be empty".to_string()));
        }

        if name.len() > 64 {
            return Err(CoreError::ValidationError("Collection name cannot exceed 64 characters".to_string()));
        }
        ...
        if !name.chars().next().unwrap_or('0').is_ascii_alphabetic() {
            return Err(CoreError::ValidationError("Collection name must start with a letter".to_string()));
        }

        if !name.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') {
            return Err(CoreError::ValidationError("Collection name can only contain letters, numbers, and underscores".to_string()));
        }

        // Reserved names
        let reserved_names = ["users", "collections", "collection_fields", "audit_log"];
        if reserved_names.contains(&name) {
            return Err(CoreError::ValidationError(format!("Collection name '{}' is reserved", name)));
        }
    ...
    fn validate_field_name(&self, name: &str) -> CoreResult<()> {
        if name.is_empty() {
            return Err(CoreError::ValidationError("Field name cannot be empty".to_string()));
        }

        if name.len() > 64 {
            return Err(CoreError::ValidationError("Field name cannot exceed 64 characters".to_string()));
        }

        // Check for valid identifier
        if !name.chars().next().unwrap_or('0').is_ascii_alphabetic() {
            return Err(CoreError::ValidationError("Field name must start with a letter".to_string()));
        }

        if !name.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') {
            return Err(CoreError::ValidationError("Field name can only contain letters, numbers, and underscores".to_string()));
        }

        // Reserved field names
        let reserved_names = ["id", "created_at", "updated_at"];
        if reserved_names.contains(&name) {
            return Err(CoreError::ValidationError(format!("Field name '{}' is reserved", name)));
        }
    ...
    fn validate_schema(&self, schema: &CollectionSchema) -> CoreResult<()> {
        ...
        for field in &schema.fields {
            ...
            if !field_names.insert(&field.name) {
                return Err(CoreError::ValidationError(format!("Duplicate field name: {}", field.name)));
            }
    ...
    fn validate_field_type(&self, field_type: &FieldType, options: &Option<FieldOptions>) -> CoreResult<()> {
        match field_type {
            FieldType::Text => {
                if let Some(opts) = options {
                    if let Some(min_len) = opts.min_length {
                        if min_len > 10000 {
                            return Err(CoreError::ValidationError("Text field min_length cannot exceed 10000".to_string()));
                        }
                    }
                    if let Some(max_len) = opts.max_length {
                        if max_len > 100000 {
                            return Err(CoreError::ValidationError("Text field max_length cannot exceed 100000".to_string()));
                        }
                    }
                    if let (Some(min), Some(max)) = (opts.min_length, opts.max_length) {
                        if min > max {
                            return Err(CoreError::ValidationError("Text field min_length cannot be greater than max_length".to_string()));
                        }
                    }
                }
            }
            FieldType::Number => {
                if let Some(opts) = options {
                    if let (Some(min), Some(max)) = (opts.min_value, opts.max_value) {
                        if min > max {
                            return Err(CoreError::ValidationError("Number field min_value cannot be greater than max_value".to_string()));
                        }
                    }
                }
            }
            FieldType::Relation { target_collection, .. } => {
                if target_collection.is_empty() {
                    return Err(CoreError::ValidationError("Relation field must specify target_collection".to_string()));
                }
            ...
            FieldType::File { max_size, allowed_types } => {
                if let Some(size) = max_size {
                    if *size > 100 * 1024 * 1024 { // 100MB limit
                        return Err(CoreError::ValidationError("File field max_size cannot exceed 100MB".to_string()));
                    }
                }
                if let Some(types) = allowed_types {
                    for mime_type in types {
                        if !mime_type.contains('/') {
                            return Err(CoreError::ValidationError(format!("Invalid MIME type: {}", mime_type)));
                        }
                    }
                }
            }
    ...
    fn generate_json_schema(&self, schema: &CollectionSchema) -> CoreResult<Value> {
    ...
    fn validate_against_json_schema(&self, data: &Value, schema: &Value) -> CoreResult<()> {
        ...
        if !data.is_object() {
            return Err(CoreError::ValidationError("Record data must be an object".to_string()));
        }
            ...
            for req_field in required {
                if let Some(field_name) = req_field.as_str() {
                    if !data_obj.contains_key(field_name) {
                        return Err(CoreError::ValidationError(format!("Required field '{}' is missing", field_name)));
                    }
                }
            }
        }
            ...
            match &field.field_type {
                FieldType::Text => {
                    if !field_value.is_string() {
                        return Err(CoreError::ValidationError(format!("Field '{}' must be a string", field.name)));
                    }
                    if let Some(options) = &field.options_json {
                        let text = field_value.as_str().unwrap();
                        if let Some(min_len) = options.min_length {
                            if text.len() < min_len {
                                return Err(CoreError::ValidationError(format!("Field '{}' is too short (minimum {} characters)", field.name, min_len)));
                            }
                        }
                        if let Some(max_len) = options.max_length {
                            if text.len() > max_len {
                                return Err(CoreError::ValidationError(format!("Field '{}' is too long (maximum {} characters)", field.name, max_len)));
                            }
                        }
                    }
                }
                FieldType::Number => {
                    if !field_value.is_number() {
                        return Err(CoreError::ValidationError(format!("Field '{}' must be a number", field.name)));
                    }
                    if let Some(options) = &field.options_json {
                        let num = field_value.as_f64().unwrap();
                        if let Some(min_val) = options.min_value {
                            if num < min_val {
                                return Err(CoreError::ValidationError(format!("Field '{}' is too small (minimum {})", field.name, min_val)));
                            }
                        }
                        if let Some(max_val) = options.max_value {
                            if num > max_val {
                                return Err(CoreError::ValidationError(format!("Field '{}' is too large (maximum {})", field.name, max_val)));
                            }
                        }
                    }
                }
                FieldType::Boolean => {
                    if !field_value.is_boolean() {
                        return Err(CoreError::ValidationError(format!("Field '{}' must be a boolean", field.name)));
                    }
                }
                FieldType::Email => {
                    if !field_value.is_string() {
                    ...
                    if !email.contains('@') || !email.contains('.') {
                        return Err(CoreError::ValidationError(format!("Field '{}' must be a valid email address", field.name)));
                    }
                }
                FieldType::Url => {
                    if !field_value.is_string() {
                    ...
                    if !url.starts_with("http://") && !url.starts_with("https://") {
                        return Err(CoreError::ValidationError(format!("Field '{}' must be a valid URL", field.name)));
                    }
                }
                FieldType::Relation { .. } => {
                    if !field_value.is_string() {
                        return Err(CoreError::ValidationError(format!("Field '{}' must be a string (UUID)", field.name)));
                    }
                    let id_str = field_value.as_str().unwrap();
                    if Uuid::parse_str(id_str).is_err() {
                        return Err(CoreError::ValidationError(format!("Field '{}' must be a valid UUID", field.name)));
                    }
                }
...
mod tests {
    use super::*;
    use crate::{Database, repository::CollectionRepository, AccessRules};
    use serde_json::json;
    ...
    async fn test_add_field_to_collection() {
        ...
        let updated_collection = service.add_field("test_posts", field_request).await.unwrap();
    ...
    async fn test_field_type_validation() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("min_length cannot be greater than max_length"));
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("min_value cannot be greater than max_value"));
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("must specify target_collection"));
    ...
    async fn test_json_schema_generation() {
        ...
        schema.add_field(Field::new(
            ...
            "title".to_string(),
            FieldType::Text,
        ).required().with_options(FieldOptions {
            min_length: Some(1),
            max_length: Some(100),
            ..Default::default()
        }));
    ...
    async fn test_record_data_validation() {
        ...
        let result = service.validate_record_data("test_validation", &valid_data).await;
        assert!(result.is_ok());
        ...
        let result = service.validate_record_data("test_validation", &invalid_data).await;
        assert!(result.is_err());
        ...
        let result = service.validate_record_data("test_validation", &invalid_data).await;
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("valid email address"));
    ...
    async fn test_sql_table_generation() {
        ...
        assert!(create_sql.contains("FOREIGN KEY (author_id) REFERENCES records_users(id)"));
    ...
    async fn test_field_management() {
        ...
        let updated = service.add_field("field_test", field_request).await.unwrap();
