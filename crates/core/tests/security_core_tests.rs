...
async fn test_audit_logger_filtering() {
    ...
    // Filter by user1
    let user1_logs = logger
        .get_audit_logs(Some(user1), None, None, None, None, 10, 0)
        .await
        .unwrap();
    ...
    // Filter by action
    let login_logs = logger
        .get_audit_logs(None, Some(AuditAction::UserLogin), None, None, None, 10, 0)
        .await
        .unwrap();
...
fn test_pii_utils_partial_redaction() {
    ...
    assert_eq!(
        PiiUtils::redact_email_partial("invalid-email"),
        "[REDACTED_EMAIL]"
    );
    ...
    assert_eq!(
        PiiUtils::redact_credit_card_partial("4111111111111111"),
        "****-****-****-1111"
    );
    assert_eq!(
        PiiUtils::redact_credit_card_partial("123"),
        "[REDACTED_CARD]"
    );
...
fn test_audit_action_serialization() {
    ...
    assert_eq!(
        AuditAction::AuthenticationFailure.as_str(),
