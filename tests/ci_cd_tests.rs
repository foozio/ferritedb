...
fn test_github_actions_workflow_syntax() {
    ...
    assert!(
        Path::new(ci_workflow_path).exists(),
        "CI workflow file should exist"
    );

    let ci_content = fs::read_to_string(ci_workflow_path).expect("Failed to read CI workflow file");

    let ci_yaml: serde_yaml::Value =
        serde_yaml::from_str(&ci_content).expect("CI workflow should be valid YAML");
    ...
    assert!(
        Path::new(release_workflow_path).exists(),
        "Release workflow file should exist"
    );

    let release_content =
        fs::read_to_string(release_workflow_path).expect("Failed to read release workflow file");

    let release_yaml: serde_yaml::Value =
        serde_yaml::from_str(&release_content).expect("Release workflow should be valid YAML");

    // Verify required fields
    assert!(
        release_yaml["name"].is_string(),
        "Release workflow should have a name"
    );
    assert!(
        release_yaml["on"].is_mapping(),
        "Release workflow should have triggers"
    );
    assert!(
        release_yaml["jobs"].is_mapping(),
        "Release workflow should have jobs"
    );
}
...
fn test_dependabot_configuration() {
    ...
    assert!(
        Path::new(dependabot_path).exists(),
        "Dependabot config should exist"
    );

    let dependabot_content =
        fs::read_to_string(dependabot_path).expect("Failed to read dependabot config");

    let dependabot_yaml: serde_yaml::Value =
        serde_yaml::from_str(&dependabot_content).expect("Dependabot config should be valid YAML");
    ...
    assert!(
        dependabot_yaml["updates"].is_sequence(),
        "Should have updates configuration"
    );

    let updates = dependabot_yaml["updates"].as_sequence().unwrap();

    // Check for Cargo ecosystem
    let has_cargo = updates
        .iter()
        .any(|update| update["package-ecosystem"].as_str() == Some("cargo"));
    assert!(has_cargo, "Should monitor Cargo dependencies");

    // Check for GitHub Actions ecosystem
    let has_github_actions = updates
        .iter()
        .any(|update| update["package-ecosystem"].as_str() == Some("github-actions"));
...
fn test_pull_request_template_exists() {
    ...
    assert!(
        Path::new(pr_template).exists(),
        "Pull request template should exist"
    );

    let content = fs::read_to_string(pr_template).expect("Failed to read PR template");

    // Check for required sections
    assert!(
        content.contains("## Description"),
        "PR template should have Description section"
    );
    assert!(
        content.contains("## Type of Change"),
        "PR template should have Type of Change section"
    );
    assert!(
        content.contains("## Testing"),
        "PR template should have Testing section"
    );
    assert!(
        content.contains("## Checklist"),
        "PR template should have Checklist section"
    );
}
...
fn test_project_documentation_exists() {
    ...
    for doc in &required_docs {
        ...
        let content = fs::read_to_string(doc).unwrap_or_else(|_| panic!("Failed to read {}", doc));
...
fn test_cargo_toml_configuration() {
    ...
    let cargo_toml: toml::Value =
        toml::from_str(&content).expect("Cargo.toml should be valid TOML");
    ...
    if let Some(package) = cargo_toml.get("package") {
        ...
        assert!(
            package.get("authors").is_some(),
            "Package should have authors"
        );
        assert!(
            package.get("license").is_some(),
            "Package should have a license"
        );
    }
}
...
fn test_dockerfile_best_practices() {
    ...
    assert!(
        dockerfile_content.contains("as builder") || dockerfile_content.contains("AS builder"),
        "Dockerfile should use multi-stage build"
    );

    // Check for non-root user
    assert!(
        dockerfile_content.contains("USER"),
...
fn test_ci_workflow_jobs() {
    let ci_content =
        fs::read_to_string(".github/workflows/ci.yml").expect("Failed to read CI workflow");

    let ci_yaml: serde_yaml::Value =
        serde_yaml::from_str(&ci_content).expect("CI workflow should be valid YAML");

    let jobs = ci_yaml["jobs"]
        .as_mapping()
    ...
    if let Some(test_job) = jobs.get(serde_yaml::Value::String("test".to_string())) {
        let steps = test_job["steps"]
            .as_sequence()
            .expect("Test job should have steps");

        let has_checkout = steps
            .iter()
            .any(|step| matches!(step["uses"].as_str(), Some(uses) if uses.contains("checkout")));
        assert!(has_checkout, "Test job should checkout code");

        let has_rust_setup = steps.iter().any(
            |step| matches!(step["uses"].as_str(), Some(uses) if uses.contains("rust-toolchain")),
        );
...
fn test_release_workflow_configuration() {
    ...
    if let Some(build_job) = jobs.get(serde_yaml::Value::String("build-release".to_string())) {
        ...
        if let Some(strategy) = strategy {
            ...
            assert!(
                matrix.is_some(),
                "Build job should use matrix strategy for multi-platform builds"
            );
        }
    }
}
...
fn test_changelog_format() {
    let changelog_content =
        fs::read_to_string("CHANGELOG.md").expect("Failed to read CHANGELOG.md");

    // Check for proper changelog format
    assert!(
        changelog_content.contains("# Changelog"),
        "Changelog should have proper title"
    );
    assert!(
        changelog_content.contains("[Unreleased]"),
        "Changelog should have Unreleased section"
    );
    assert!(
        changelog_content.contains("## ["),
        "Changelog should have version sections"
    );
...
fn test_license_file() {
    let license_content = fs::read_to_string("LICENSE").expect("Failed to read LICENSE file");
