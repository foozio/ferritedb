...
fn test_migrate_run_command() {
    ...
    assert!(stdout.contains("Migrations completed successfully"), "Should contain success message");
}
...
fn test_migrate_status_command() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("Applied migrations") || stdout.contains("Database connection is healthy"), 
...
fn test_admin_create_command() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("Admin user created successfully"), "Should contain success message");
    assert!(stdout.contains("admin@example.com"), "Should contain admin email");
}
...
fn test_admin_list_command() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("admin@example.com"), "Should list the created admin user");
    assert!(stdout.contains("Found 1 users") || stdout.contains("admin"), "Should show user count or admin role");
}
...
fn test_gen_jwt_command() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("JWT tokens generated successfully"), "Should contain success message");
    assert!(stdout.contains("Access Token:"), "Should contain access token");
    assert!(stdout.contains("Refresh Token:"), "Should contain refresh token");
    assert!(stdout.contains("admin@example.com"), "Should contain user email");
}
...
fn test_import_export_json_data() {
    ...
    std::fs::write(&import_file, serde_json::to_string_pretty(&test_data).unwrap()).unwrap();
    ...
    assert!(stderr.contains("Collection 'posts' not found") || stdout.contains("Collection 'posts' not found"),
...
fn test_export_nonexistent_collection() {
    ...
    // Should contain error about collection not found
    assert!(stderr.contains("Collection 'nonexistent_collection' not found") || 
...
fn test_gen_jwt_nonexistent_user() {
    ...
    assert!(stderr.contains("User 'nonexistent@example.com' not found") || 
...
fn test_admin_create_duplicate_user() {
    ...
    assert!(stderr.contains("already exists") || stdout.contains("already exists"),
...
fn test_migrate_revert_command() {
    ...
    assert!(stderr.contains("not supported") || stdout.contains("not supported"),
